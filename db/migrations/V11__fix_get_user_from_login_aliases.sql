...
BEGIN
    ...
    IF FOUND THEN
        ...
        WHERE provider = p_provider AND provider_user_id = p_provider_user_id
          AND (provider_username IS DISTINCT FROM p_provider_username
            OR provider_display_name IS DISTINCT FROM p_provider_display_name);

        RETURN QUERY SELECT v_user_id AS id, v_user_name AS name;
    ELSE
        -- Create new user and account
        INSERT INTO users (name)
        VALUES (p_provider_display_name)
        RETURNING id, name INTO v_user_id, v_user_name;
